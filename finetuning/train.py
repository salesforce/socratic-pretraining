...
class DataTrainingArguments:
    ...
    split_question_summary_evaluation: bool = field(
        default=False, metadata={"help": "Whether to split the evaluation of questions and summary."}
    )
    
    mode_token: Optional[str] = field(
        default=None, metadata={"help": "Token to be added in front of the query for every chunk in the multi encoder dataset"}
    )
    
    store_predictions_as_json: bool = field(
        default=False, metadata={"help": "Whether to store the predictions in json format instead of text format (useful when generations have multiple lines)"}
    )

    provided_decoder_input: bool = field(
        default=False, metadata={"help": "Pass input to the decoder. The model is not trained to predict whatever comes before the <qsep> in the target text."} 
    )
...
def main():
    ...
    if model_args.model_name_or_path == 'facebook/bart-large':
        tokenizer.add_tokens(['<qmask>', '<qsep>', '<reconstruct>', '<answer>', '<ask&answer>', '<ask>'])
    tokenizer.add_tokens(['<endsum>', '<generatesummary>', '<generatequestions>'])
        ...
        if training_args.do_train:
            ...
            if data_args.train_file.endswith('.pickle'):
                ...
                train_dataset = MultiEncoderDataset(
                    ...
                    mode_token=data_args.mode_token,
                    provided_decoder_input=data_args.provided_decoder_input,
                )
    ...
    def split_questions_summaries(texts):
        summaries, questions = [], []
        for text in texts:
            summary, question = text, '' 
            if '<qsep>' in text or '||' in text:
                if '<qsep>' in text:
                    text_split = text.split('<qsep>')
                else:
                    text_split = text.split('||')
                question = text_split[0].strip()
                summary = text_split[1].strip()
            summaries.append(summary)
            questions.append(question)
        return questions, summaries

    def separate_sents_newline(texts):
        return ["\n".join(nltk.sent_tokenize(text)) for text in texts]

    def postprocess_text_question_summary(preds, labels):
        preds = [pred.strip() for pred in preds]
        labels = [label.strip() for label in labels]

        preds_questions, preds_summaries = split_questions_summaries(preds)
        labels_questions, labels_summaries = split_questions_summaries(labels)

        # rougeLSum expects newline after each sentence
        preds_questions = separate_sents_newline(preds_questions)
        preds_summaries = separate_sents_newline(preds_summaries)
        labels_questions = separate_sents_newline(labels_questions)
        labels_summaries = separate_sents_newline(labels_summaries)

        return preds_summaries, labels_summaries, preds_questions, labels_questions
    ...
    def compute_metrics(eval_preds):
        ...
        if not data_args.split_question_summary_evaluation:
            # Some simple post-processing
            decoded_preds, decoded_labels = postprocess_text(decoded_preds, decoded_labels)

        else:
            # Some simple post-processing
            decoded_preds, decoded_labels, decoded_pred_questions, decoded_label_questions = postprocess_text_question_summary(decoded_preds, decoded_labels)
            # Questions
            question_result = metric.compute(predictions=decoded_pred_questions, references=decoded_label_questions, use_stemmer=True)
            question_result = {f'questions_{key}': value.mid.fmeasure * 100 for key, value in question_result.items()}
            question_result['eval_questions_mean_rouge'] = mean([question_result[k] for k in ['questions_rouge1', 'questions_rouge2', 'questions_rougeLsum']])
        ...
        if data_args.split_question_summary_evaluation:
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                with open(prediction_path, "w") as writer:
                    if data_args.store_predictions_as_json:
